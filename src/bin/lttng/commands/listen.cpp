...
public:
	...
	void print_trigger_notification()
	{
		...
		case LTTNG_CONDITION_TYPE_SESSION_ROTATION_COMPLETED:
			 ...
			 * trigger types
			 */
			_unpack_and_print_session_rotation_trigger_notification();
			break;
...
private:
	void _unpack_and_print_session_consumed_size_trigger_notification()
	{
		...
		const enum lttng_condition_status get_session_name_status =
		...
		const enum lttng_condition_status get_threshold_status =
	...
	void _unpack_and_print_buffer_usage_trigger_notification()
	{
		...
		const enum lttng_condition_status get_channel_name_status =
		...
		const enum lttng_condition_status get_domain_type_status =
		 ...
		 */
		double usage_ratio;
		const enum lttng_evaluation_status get_usage_ratio_status =
			lttng_evaluation_buffer_usage_get_usage_ratio(_trigger_evaluation,
								      &usage_ratio);
		if (get_usage_ratio_status != LTTNG_EVALUATION_STATUS_OK) {
			ERR_FMT("Failed to get usage ratio of the buffer associated with the trigger.");
			LTTNG_THROW_ERROR(lttng::format(
		...
		const enum lttng_evaluation_status get_usage_status =
			lttng_evaluation_buffer_usage_get_usage(_trigger_evaluation, &usage_bytes);
		if (get_usage_status != LTTNG_EVALUATION_STATUS_OK) {
			ERR_FMT("Failed to get usage of the buffer (in bytes) associated with the trigger.");
			LTTNG_THROW_ERROR(lttng::format(
		...
		if (_trigger_type == LTTNG_CONDITION_TYPE_BUFFER_USAGE_LOW) {
			if (is_threshold_ratio) {
				_print_buffer_usage_low_trigger_notification_threshold_ratio(
					channel_name,
					session_name,
					tracer_domain_type,
					threshold_ratio,
				...
				_print_buffer_usage_low_trigger_notification_threshold(
					channel_name,
					session_name,
					tracer_domain_type,
					threshold,
			...
			if (is_threshold_ratio) {
				_print_buffer_usage_high_trigger_notification_threshold_ratio(
					channel_name,
					session_name,
					tracer_domain_type,
					threshold_ratio,
				...
				_print_buffer_usage_high_trigger_notification_threshold(
					channel_name,
					session_name,
					tracer_domain_type,
					threshold,
	...
	void _unpack_and_print_session_rotation_trigger_notification()
	{
		const char *session_name_char_array;
		const enum lttng_condition_status get_session_name_status =
			lttng_condition_session_rotation_get_session_name(_trigger_condition,
									  &session_name_char_array);
		if (get_session_name_status != LTTNG_CONDITION_STATUS_OK) {
			ERR_FMT("Failed to get session name associated with the trigger.");
			LTTNG_THROW_ERROR(lttng::format(
		...
		const enum lttng_evaluation_status get_id_status =
			lttng_evaluation_session_rotation_get_id(_trigger_evaluation, &rotation_id);
		if (get_id_status != LTTNG_CONDITION_STATUS_OK) {
			ERR_FMT("Failed to get the rotation id associated with the trigger.");
			LTTNG_THROW_ERROR(lttng::format(
				"Failed to get the rotation id associated with the trigger."));
		}

		switch (_trigger_type) {
		case LTTNG_CONDITION_TYPE_SESSION_ROTATION_ONGOING:
			_print_session_rotation_ongoing_trigger_notification(session_name,
									     rotation_id);
			break;
		case LTTNG_CONDITION_TYPE_SESSION_ROTATION_COMPLETED:
		{
			...
			const enum lttng_evaluation_status get_location_status =
				lttng_evaluation_session_rotation_completed_get_location(
					_trigger_evaluation, &trace_chunk_archive_location);
			if (get_location_status != LTTNG_EVALUATION_STATUS_OK) {
				ERR_FMT("Failed to get the archive location associated with the trigger.");
				LTTNG_THROW_ERROR(lttng::format(
					"Failed to get the archive location associated with the trigger."));
			}

			// Don't pass the archive location by copying it?
			_print_session_rotation_completed_trigger_notification(
				session_name, rotation_id, trace_chunk_archive_location);
			break;
		}
		default:
	...
	void _print_buffer_usage_low_trigger_notification_threshold_ratio(
		std::string channel_name,
		std::string session_name,
		lttng_domain_type tracer_domain_type,
		double threshold_ratio,
	...
	void _print_buffer_usage_low_trigger_notification_threshold(
		std::string channel_name,
		std::string session_name,
		lttng_domain_type tracer_domain_type,
		uint64_t threshold,
	...
	void _print_buffer_usage_high_trigger_notification_threshold_ratio(
		std::string channel_name,
		std::string session_name,
		lttng_domain_type tracer_domain_type,
		double threshold_ratio,
	...
	{
		fmt::println(
			"Trigger notification: The amount of data in a buffer belonging to channel `{}` of session `{}` is higher than the set maximum threshold ratio of {}. Usage ratio: {}, Buffer usage: {} bytes)",
			channel_name,
			session_name,
			threshold_ratio,
			usage_ratio,
	...
	void _print_buffer_usage_high_trigger_notification_threshold(
		std::string channel_name,
		std::string session_name,
		lttng_domain_type tracer_domain_type,
		uint64_t threshold,
	...
	{
		fmt::println(
			"Trigger notification: The amount of data in a buffer belonging to channel `{}` of session `{}` is higher than the set maximum threshold of {} bytes. Usage ratio: {}, Buffer usage: {} bytes)",
			channel_name,
			session_name,
			threshold,
			usage_ratio,
	...
	void _print_session_rotation_ongoing_trigger_notification(std::string session_name,
								  uint64_t rotation_id)
	{
		fmt::println(
			"Trigger notification: Session rotation (id: {}) started for session `{}`.",
			rotation_id,
			session_name);
	}
	...
	{
		fmt::println(
			"Trigger notification: Session rotation (id: {}) completed for session `{}`. The resulting trace chunks are located here:",
			rotation_id,
			session_name);
...
int cmd_listen(int argc, const char **argv)
{
	...
	/* Determine whether to listen for all trigger notifications */
	...
	if (listen_all_triggers) {
		 ...
		 *
		 * For each requested trigger name, check if there is an
		 * existing trigger that matches.
		 */
